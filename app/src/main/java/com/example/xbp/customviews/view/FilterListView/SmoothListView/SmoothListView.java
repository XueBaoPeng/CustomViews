...
public class SmoothListView  extends ListView implements AbsListView.OnScrollListener {
    ...
    private Scroller mScroller;//used for scroll listener
    ...
    //header view contenet,use it to calculater the header's height.and hide it
    ...
    private final static int SCROLL_DURATTION=400;//scroll back duration;
    ...
    public void setRefreshEnable(boolean enable){
        ...
        if(!mEnablePullRefresh){//disable hide the countent
    ...
    private void updateHeaderHeight(float delta){
        mHeaderView.setViesiableHeight((int) (delta+mHeaderView.getVisiableHeight()));
        if(mEnablePullRefresh&&!mPullLoading){
            //未处于刷新状态更新箭头
            if (mHeaderView.getVisiableHeight() > mHeaderViewHeight) {
                mHeaderView.setState(SmoothListViewHeader.START_READY);
            } else {
                mHeaderView.setState(SmoothListViewHeader.START_NORMAL);
            }
        }
    ...
    private void resetFooterHeight() {
        ...
        if (bottomMargin > 0) {
            ...
            mScroller.startScroll(0, bottomMargin, 0, -bottomMargin, SCROLL_DURATTION);
            invalidate();
        }
    }
            ...
            default:
                ...
                if (getFirstVisiblePosition() == 0) {
                    ...
                    if (mEnablePullRefresh
                        ...
                        mHeaderView.setState(SmoothListViewHeader.START_REFRESHING);
                        if (mListViewListener != null) {
    ...
    public void computeScroll() {
        if(mScroller.computeScrollOffset()){
            if(mScrollBack==SCROLLBACK_HEADER){
                mHeaderView.setViesiableHeight(mScroller.getCurrY());
            }else {
